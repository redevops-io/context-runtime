"""redevops-rag × ContextOS — the second tenant: tune retrieval, not just plan it.
...
ContextOS picks a config per query intent and learns which one maximizes **retrieval
...
class ContextOSRetrieverTuner:
    """Wrap a redevops-rag ``RAG`` so ContextOS tunes its knobs per query intent.

    Usage (real):
        from redevops_rag import RAG
        tuner = ContextOSRetrieverTuner(rag=RAG(db_path="vault.duckdb", use_reranker=True))
    ...
    def search(self, query: str) -> list:
        ...
        if self.rag is None:
            raise RuntimeError("no RAG bound — install 'contextos[rag]' and pass rag=RAG(...)")
