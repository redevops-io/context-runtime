...
"""Build the FinanceBench corpus: SEC filing PDFs → tables-preserved passages.
    ...
    uv run --with pdfplumber scripts/build_corpus.py [FINANCEBENCH_DIR]

Uses PdfTableExtractor (pdfplumber) so the numbers in financial statements survive as
pipe tables instead of being flattened — the "hard to ingest" half of the benchmark.
Reads ``<dir>/pdfs/`` → writes ``<dir>/corpus/`` + ``<dir>/corpus.manifest.jsonl``.
"""
import os
import sys
import time

# repo root = three levels up from benchmarks/context-vs-model/scripts/
...
FB = os.path.abspath(sys.argv[1]) if len(sys.argv) > 1 else \
    os.path.join(os.path.dirname(__file__), "..", ".financebench")
t0 = time.time()
stats = ingest_corpus(
