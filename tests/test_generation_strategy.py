...
before (the legacy per-bucket default strategy from rules.BUCKET_DEFAULTS). On → the `reason` step
becomes a bandit arm: one candidate per intent strategy, carrying its thinking/budget, and the bandit
arm key distinguishes strategies.
"""
...
def test_disabled_keeps_legacy_bucket_default(monkeypatch):
    # Off -> byte-identical legacy plan: one strategy per candidate = the bucket's BUCKET_DEFAULTS
    # strategy, with NO generation-strategy expansion and no thinking/budget params. (The default is
    # not universally single_shot: reasoning-heavy buckets like multi_hop/synthesis default to
    # plan_worker_critic; the contract is "the legacy bucket default," whatever it is.)
    from context_runtime.planner import rules
    ...
    expected = rules.BUCKET_DEFAULTS[intent.bucket][1]   # multi_hop -> plan_worker_critic
    cands = rt.candidates.generate(intent, g)
    assert cands and all(_reason_strategy(c) == expected for c in cands)
