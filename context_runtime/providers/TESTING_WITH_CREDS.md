...
| **DO GradientModel** (serverless inference) | ✅ **adapter correct**; auth path fully exercised. Findings: (1) inference needs a **model access key**, not the API token (token → 402; create-key API is retired-410, create in the control panel). (2) **VPC-restricted** keys require the request over DO's private network — enable the **VPC-local DNS resolver** (the *second-to-last IP of the VPC CIDR*; for `10.120.0.0/20` → `10.120.15.254`). On DOKS set the pod `dnsConfig.nameservers` to it; the request then goes private (verified: `connected_ip=10.120.15.254`), and `hostNetwork: true` sources from the node's VPC IP. (3) Even with the private path + VPC-IP source, the test key 401s on a plain `GET /v1/models` → the **key value** must be re-copied from the console (the VPC side is satisfied; auth still fails, which is key-value, not networking or adapter). |
