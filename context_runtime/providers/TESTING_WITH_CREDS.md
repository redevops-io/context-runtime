...
| **DO GradientModel** (serverless inference) | ✅ **adapter correct** — key transmitted properly, verified from two origins (evo-x2 and an in-VPC DOKS pod; `keylen` correct, clean app-level responses). Blocked only by DO's VPC enforcement, not code. Gotchas found: (1) inference needs a **model access key**, not the API token (token → misleading 402; create-key API is retired-410, use the control panel). (2) If the account only issues **VPC-restricted** keys, the request must reach inference over DO's **private** network — a DOKS pod hitting public `inference.do-ai.run` egresses via the node's **public IP**, so DO returns **401** even from inside the VPC. Reaching it privately (a private inference endpoint / VPC route) is a DO networking config, independent of this adapter. |
