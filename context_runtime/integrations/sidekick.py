"""sidekick × Context Runtime — optimize sidekick's self-learning via context management.

sidekick learns *skills* (object level). This makes Context Runtime learn *which skills/context
to surface, and how much budget to spend* (meta level), using sidekick's own acceptance
metric as the reward. Two pieces:

  * ``ContextRuntimeSkillStore`` — a DROP-IN for sidekick's ``SkillStore`` (same
    save/all/recall/record_use interface), but ``recall()`` ranks via Context Runtime's
...
class SubtaskOutcome:
    """The subset of sidekick.metrics.SubtaskRecord Context Runtime needs as 'actuals'."""
...
class ContextRuntimeSkillStore:
    """Drop-in for sidekick.skills.SkillStore, backed by Context Runtime recall + learning.
    ...
    def recall(self, query: str, limit: int = 3) -> list[Skill]:
        """Context Runtime-planned recall: classify the task, let the bandit pick a Strategy,
...
def make_skill_store(skills_dir, use_context_runtime: bool = True, **kw):
    """Factory for the orchestrator.py:186 swap. ``use_context_runtime=False`` returns sidekick's
    own SkillStore so the change is a safe, reversible A/B toggle."""
    if use_context_runtime:
        return ContextRuntimeSkillStore(skills_dir, **kw)
