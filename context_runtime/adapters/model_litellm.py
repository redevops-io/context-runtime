...
native cost-tiered routing policy (``Tier`` below) — Context Runtime is the control plane, so
...
Install:  pip install "context_runtime[litellm]"
"""
...
class LiteLLMModel:
    ...
    def _litellm(self):
        try:
            ...
            raise RuntimeError(
                "LiteLLMModel needs the 'litellm' extra: pip install 'context_runtime[litellm]'"
            ) from e
        return litellm
